...
- **Chunks are longer than the embedding model reads.** Chunks target about 500 tokens, but this model's tokenizer truncates input at 128 tokens (`truncation.max_length` in `src/data/local-models/.../tokenizer.json`), so only the beginning of a long chunk shapes its embedding. The lexical boost and heading matching partly compensate, which is why they exist, but smaller chunks (around 100 tokens) or a model with a longer window are the first things I would try. I have not measured the effect on retrieval quality.
